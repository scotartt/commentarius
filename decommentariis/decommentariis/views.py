...
from decommentariis.forms import CohortEditForm, CohortCreateForm
...
class TextListView(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		authors_queryset = TEIEntry.objects.exclude(
			author=None).order_by('author').values_list('author', flat=True).distinct()
		context['author_list'] = authors_queryset
...
class UserCommentaryView(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		commentaries = []
		for commentary_entry in context['object_list']:
			entry = {}
			entry['commentary_html'] = markdown.markdown(
				commentary_entry.commentary,
				encoding="utf-8", output_format="html5", safe_mode=False)
			entry['commentary_md'] = commentary_entry.commentary
			entry['username'] = commentary_entry.user.username
			entry['votes'] = commentary_entry.votes
			entry['commentary_date'] = commentary_entry.creation_date
			entry['section'] = commentary_entry.section
			entry['section_text'] = commentary_entry.section.readData()
...
class CohortListView(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		instructors_queryset = Cohort.objects.exclude(
			instructor=None).order_by(
			'instructor__username').values_list('instructor__username', flat=True).distinct()
		context['instructor_list'] = instructors_queryset
